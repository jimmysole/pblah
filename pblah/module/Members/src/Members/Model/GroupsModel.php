...
class GroupsModel extends Groups
{
    ...
    public function getGroupInformation($group_id)
    {
        ...
        $query_group_admin = parent::$sql->getAdapter()
        ->query(parent::$sql->buildSqlString($select_admins),
        ...
        ->join(array(
            'm' => 'members'
        ),  'g.member_id = m.id', array('username'))
        ->join(array(
            'grp' => 'groups'
        ),  'g.group_id = grp.id')
        ->where(array(
        ...
        $query = parent::$sql->getAdapter()
        ->query(parent::$sql->buildSqlString($select),
            Adapter::QUERY_MODE_EXECUTE);
